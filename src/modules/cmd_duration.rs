...
fn undistract_me<'a>(
    ...
    if config.show_notifications && config.min_time_to_notify as u128 <= elapsed {
        ...
        let used_osc =
            add_terminal_notification(&mut module, context, &body, config.notification_timeout);
...
impl OscType {
    fn format(&self, body: &str, _timeout_ms: Option<u32>) -> String {
        match self {
...
fn add_terminal_notification(
    ...
    if let Some(osc) = osc_type {
        module
            .segments
            .push(Segment::control(osc.format(body, timeout_ms)));
