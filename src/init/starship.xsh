...
def starship_prompt():
    ...
    $STARSHIP_JOBS = sum(1 for job in __xonsh__.all_jobs.values() if job['obj'] and job['obj'].poll() is None)
    ...
    return $(::STARSHIP:: prompt --status=@(status) --cmd-duration=@(duration) | cat)
...
def starship_rprompt():
    ...
    duration = round((last_cmd.ts[1] - last_cmd.ts[0]) * 1000) if last_cmd else 0
    # The `| cat` is a workaround for https://github.com/xonsh/xonsh/issues/3786. See https://github.com/starship/starship/pull/2807#discussion_r667316323.
    return $(::STARSHIP:: prompt --status=@(status) --cmd-duration=@(duration) --right | cat)
