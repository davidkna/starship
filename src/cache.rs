...
impl Cache {
    ...
    pub fn get(&mut self, binary_path: &Path, command: &str) -> Option<&Output> {
    ...
    pub fn set(&mut self, binary_path: &Path, command: &str, output: &Output) {
        ...
        bin.commands.insert(command.to_owned(), output.to_owned());
        self.changed = true;
    }
...
struct BinaryCache {
    ...
    commands: HashMap<FullCommand, Output>,
}
