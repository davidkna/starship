...
| `when`        | `false`                         | Either a boolean (`true` or `false`) to permanently enable or disable the module or a string shell command used as a condition to show the module. The module will be shown if the command returns a `0` status code. |
